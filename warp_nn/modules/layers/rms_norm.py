...
from warp_nn.initializers import ones
...
class RMSNorm(Module):
    def __init__(
        ...
        *,
        ...
        r"""Apply Root Mean Square (RMS) Normalization over the trailing dimensions of the input.

        .. math::

            \text{RMSNorm}(x) = \frac{x}{\sqrt{\text{E}[x^2] + \epsilon}} \, \gamma

        The mean square is computed over the last ``len(normalized_shape)`` dimensions.
        ...
        The weights are initialized to ones.

        |hr|

        :param normalized_shape: The shape of the trailing dimensions to normalize over.
        :param eps: The value added to the mean square for numerical stability.
            If None, the machine epsilon of the 32-bit floating-point type is used.
        :param elementwise_affine: Whether to include learnable per-element weights.
        ...
        self.eps = float(np.finfo(np.float32).eps) if eps is None else eps
    ...
    def __call__(self, input: wp.array) -> wp.array:
        ...
        if shape not in self._cache:
            ...
            moments = Moments(rows, features, centered=False, device=self.device, requires_grad=self.requires_grad)
            self._cache[shape] = (output, moments)
        output, moments = self._cache[shape]
        # launch kernels (the variance of the non-centered input is its mean square)
