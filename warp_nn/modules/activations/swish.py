...
def _create_kernels(config: KernelConfig, *, alpha: float):
    ...
    def activation(x: Any):
        # x * sigmoid(alpha * x), keeping the exponent non-positive to avoid overflowing exp()
        z = x.dtype(wp.static(alpha)) * x
        if z < x.dtype(0.0):
            e = wp.exp(z)
...
class Swish(Module):
    def __init__(self, *, alpha: float = 1.0, requires_grad: bool = True) -> None:
        ...
        This class computes the element-wise Swish activation function:
    ...
    def alpha(self) -> float:
        """The coefficient that multiplies the input of the sigmoid function."""
        return self._alpha
