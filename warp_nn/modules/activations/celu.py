...
def _create_kernels(config: KernelConfig, *, alpha: float):
    ...
    def activation(x: Any):
        if x >= x.dtype(0.0):
            return x
        a = x.dtype(wp.static(alpha))
        return a * (wp.exp(x / a) - x.dtype(1.0))
...
class CELU(Module):
    def __init__(self, *, alpha: float = 1.0, requires_grad: bool = True) -> None:
        r"""Continuously differentiable Exponential Linear Unit (CELU) activation function.

        This class computes the element-wise CELU activation function:

        .. math::

            \text{CELU}(x) = \max(0, x) + \min\left(0, \alpha \, (e^{x / \alpha} - 1)\right)

        :param alpha: The alpha value for the CELU function. It must be non-zero.
    ...
    def alpha(self) -> float:
        """The alpha value for the CELU function."""
        return self._alpha
